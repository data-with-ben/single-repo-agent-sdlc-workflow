...
description: Manage backlog tasks using the Backlog.md CLI tool. Create, edit, assign, prioritize, and track tasks with full metadata.
---
# Instructions for the usage of Backlog.md CLI Tool

Never update backlog tasks by editing markdown files directly. Always use the `backlog` CLI commands to ensure metadata synchronization and proper task management.
...
- IMPORTANT: The backlog lives at `backlog/` in the workspace root. Run all `backlog` CLI commands from inside that directory:
  ```bash
  cd backlog
  ```
...
For Implementation Notes specifically, prefer the helper script — it splits on real newlines and runs one `--append-notes` per line, sidestepping shell-quoting issues entirely:

```bash
# From a file
bash .claude/skills/manage-backlog-tasks/scripts/append-notes.sh 42 --file notes.md

# From stdin
printf 'line one\nline two\n' | bash .claude/skills/manage-backlog-tasks/scripts/append-notes.sh 42
```

For other multi-line fields (`--desc`, `--plan`, `--final-summary`), use one of the forms below, listed in order of preference for AI agents:
